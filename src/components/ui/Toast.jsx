    ...
    <ToastContext.Provider value={showToast}>
      ...
      <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-[999] flex flex-col items-center gap-2 pointer-events-none">
        {toasts.map(t => (
          <div
            key={t.id}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-full bg-salve-card2/95 border border-salve-lav/20 shadow-lg backdrop-blur-sm transition-all duration-300 ${t.leaving ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0 toast-enter'}`}
          >
