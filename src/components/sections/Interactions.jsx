  ...
  return (
    <div className="mt-2">
      ...
      <Card className="!p-3.5 mb-4">
        ...
        <div className="flex flex-wrap gap-1.5">
          {active.length === 0 ? (
            ...
            <span key={m.id} className="bg-salve-card2 border border-salve-border rounded-full px-2.5 py-0.5 text-[13px] text-salve-text flex items-center gap-1">
              <Pill size={10} /> {m.name}
        ...
        {rxcuiMeds.length >= 2 && (
          <button
            ...
            className="mt-2.5 bg-salve-card2 border border-salve-border rounded-lg px-3 py-1.5 text-[13px] text-salve-lav font-montserrat cursor-pointer flex items-center gap-1.5 hover:border-salve-lav/40 transition-colors disabled:opacity-50"
          >
        ...
        {liveError && <div className="text-[13px] text-salve-rose mt-1.5">{liveError}</div>}
      </Card>

      {/* Live NLM Interaction Results */}
      {liveResults.length > 0 && (
        <>
          <div className="text-[13px] font-semibold text-salve-lav uppercase tracking-widest mb-2 flex items-center gap-1.5">
          ...
          <div className="md:grid md:grid-cols-2 md:gap-4">{liveResults.map((r, i) => (
            <Card key={`live-${i}`} style={{ borderLeft: `3px solid ${r.severity === 'danger' ? C.rose : r.severity === 'caution' ? C.amber : C.sage}` }}>
              <div className="flex justify-between items-center mb-1.5">
                <span className="text-[15px] font-semibold text-salve-text">{r.pair || 'Interaction'}</span>
      ...
      {interactions.length > 0 && (
        <div className="text-[13px] font-semibold text-salve-amber uppercase tracking-widest mb-2">Local Database</div>
      )}

      {interactions.length === 0 && !liveChecked ? (
        <Card className="text-center !py-6">
          <CheckCircle size={28} className="mx-auto mb-2 text-salve-sage" />
          <div className="text-[15px] text-salve-sage font-medium mb-1">No known interactions</div>
      ...
      <p className="text-[12px] text-salve-textFaint italic text-center mt-3">
