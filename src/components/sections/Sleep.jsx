...
function SleepBar({ label, pct, color }) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-[12px] text-salve-textMid font-montserrat w-10">{label}</span>
      ...
      <span className="text-[12px] text-salve-text font-montserrat w-8 text-right">{Math.round(pct)}%</span>
    </div>
  );
}
  ...
  return (
    <div className="bg-salve-card2 rounded-lg p-2.5 text-center">
      ...
      <div className="text-sm font-medium text-salve-text font-montserrat">
        {value ?? '\u2014'}<span className="text-[12px] text-salve-textFaint ml-0.5">{unit}</span>
      </div>
  ...
  return (
    <button onClick={() => setOpen(!open)} className="w-full text-left bg-transparent border-none p-0 cursor-pointer" aria-expanded={open}>
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[12px] text-salve-lav font-montserrat font-medium">{title}</span>
        <ChevronDown size={10} className={`text-salve-lav transition-transform ${open ? 'rotate-180' : ''}`} />
      </div>
      {open && <p className="text-[12px] text-salve-textMid font-montserrat leading-relaxed mt-1.5 ml-4">{body}</p>}
    </button>
  );
}
...
function SourceBadge({ source }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-[12px] font-montserrat rounded-full px-1.5 py-0.5 bg-salve-card2" style={{ color }}>
...
export default function Sleep({ data, addItem, updateItem, removeItem, highlightId }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {lastNight && (
        <Card className="relative overflow-hidden">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[12px] text-salve-textFaint font-montserrat uppercase tracking-wider">Last Night</p>
              ...
              <p className="text-[12px] text-salve-textMid font-montserrat mt-1">{fmtDate(lastNight.date)}</p>
            </div>
      ...
      {stageBreakdown && (
        <Card>
          <p className="text-[12px] text-salve-textFaint font-montserrat uppercase tracking-wider mb-2">Sleep Stages</p>
          ...
          <div className="flex gap-4 mt-2.5 pt-2 border-t border-salve-border">
            <span className="text-[12px] text-salve-textMid font-montserrat">
      ...
      {chartData.some(d => d.hrs > 0) && (
        <Card>
          <p className="text-[12px] text-salve-textFaint font-montserrat uppercase tracking-wider mb-2">7-Day Trend</p>
      ...
      {weeklyStats && (
        <Card>
          <p className="text-[12px] text-salve-textFaint font-montserrat uppercase tracking-wider mb-2">This Week</p>
        ...
        {FILTER_PILLS.map(pill => {
          ...
          return (
            <button
              ...
              className={`text-[12px] font-montserrat px-2.5 py-1 rounded-full border transition-colors ${
                active
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <span className="text-[12px] text-salve-textFaint font-montserrat ml-2">{fmtDate(rec.date)}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5">
                    ...
                    <div className="mt-2.5 pt-2.5 border-t border-salve-border space-y-2">
                      ...
                      {confirmId === rec.id ? (
                        ...
                        <button
                          ...
                          className="text-[12px] text-salve-rose/60 hover:text-salve-rose font-montserrat bg-transparent border-none cursor-pointer p-0"
