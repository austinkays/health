              ...
              <div className={`expand-section ${isExpanded ? 'open' : ''}`}><div>
                <div className="mt-2.5 pt-2.5 border-t border-salve-border/50" onClick={e => e.stopPropagation()}>
                  ...
                  {draftResult[a.id] && (
                    <div className="mt-2 p-2.5 rounded-lg bg-salve-lav/8 border border-salve-lav/20">
                      <div className="flex items-center justify-between mb-1">
                        <div className="text-[13px] font-semibold text-salve-lav flex items-center gap-1"><FileText size={11} /> Sage Draft</div>
                        <div className="flex items-center gap-2">
                          <button onClick={() => { navigator.clipboard.writeText(draftResult[a.id]); setCopiedId(a.id); setTimeout(() => setCopiedId(null), 2000); }} className="bg-transparent border-none cursor-pointer text-salve-textFaint hover:text-salve-lav p-0 text-[13px] font-montserrat" aria-label="Copy draft to clipboard">{copiedId === a.id ? '✓ Copied' : 'Copy'}</button>
                  ...
                  <div className="flex gap-2 mt-3">
                    <button onClick={() => { setForm(a); setEditId(a.id); setSubView('form'); }} aria-label="Edit appeal" className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 bg-salve-lav/10 text-salve-lav text-xs font-semibold font-montserrat border border-salve-lav/20 cursor-pointer hover:bg-salve-lav/20 transition-colors"><Edit size={13} /> Edit</button>
                    <button onClick={() => del.ask(a.id, a.subject)} aria-label="Delete appeal" className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-salve-textFaint text-xs font-medium font-montserrat border border-salve-border cursor-pointer hover:bg-salve-rose/10 hover:text-salve-rose hover:border-salve-rose/25 transition-colors"><Trash2 size={13} /> Delete</button>
                  </div>
                </div>
              </div></div>
