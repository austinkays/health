...
│   │       ├── Journal.jsx       # Health journal entries + add/edit + tag & symptom filter pills + structured symptom builder (up to 10 per entry with individual severity 0-10 + autocomplete from conditions + COMMON_SYMPTOMS) + mood-aware reflection prompts (gentle rotating prompt above textarea via journalPrompts.js) + cross-link to conditions & medications (toggle chips) + gratitude field (mood-gated, shown for positive/neutral moods) + severity-colored symptom pills on cards + linked record navigation buttons + gratitude sparkle badge
