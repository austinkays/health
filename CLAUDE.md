...
│   ├── wearable.js               # Vercel serverless: unified OAuth2 router for ALL wearables (Oura, Dexcom, Withings, Fitbit, Whoop) via ?provider=X&action=token|refresh|data|config|status|disconnect|webhook|bootstrap_subscriptions. Consolidates 5 integrations into 1 function to stay under the Hobby tier 12-function ceiling. Each provider has its own handler block inline; shared CORS/auth/rate-limit/fetch-with-timeout boilerplate at the top. Per-user rate buckets keyed by (userId, provider). **Fitbit live-push (Phases 1-2):** token action writes to wearable_connections (migration 050) and auto-registers 3 per-user webhook subscriptions (activities/sleep/body) with unique per-collection IDs; disconnect action tears subs down + revokes token. Phase 3 (webhook → data ingest) pending tester. **Oura live-push (Phases 1-3 deployed):** token action upserts wearable_connections + fetches /personal_info for provider_user_id; bootstrap_subscriptions admin action registers 7 app-level subscriptions (sleep/daily_sleep/daily_readiness/daily_activity/daily_spo2/workout/daily_stress) in oura_app_subscriptions (migration 051); webhook handler (reachable at clean path /api/oura-webhook via vercel.json rewrite, since Oura strips query strings during verification) fetches incoming notifications by record id, transforms into vitals/activities tagged source='oura', upserts via service-role REST. Existing useRealtimeSync hook delivers inserts live to open browser tabs. Supabase service-role REST helpers (supabaseConfig, upsertWearableConnection, getWearableConnection, deleteWearableConnection, getConnectionByProviderUserId, patchConnectionTokens, touchConnectionWebhook, bulkInsertVitals, bulkInsertActivities, vitalAlreadyExists, activityAlreadyExists) at the top mirror the Terra pattern.
