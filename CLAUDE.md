...
│   ├── wearable.js               # Vercel serverless: unified OAuth2 router for ALL wearables (Oura, Dexcom, Withings, Fitbit, Whoop) via ?provider=X&action=token|refresh|data|config|status|disconnect|webhook. Consolidates 5 integrations into 1 function to stay under the Hobby tier 12-function ceiling. Each provider has its own handler block inline; shared CORS/auth/rate-limit/fetch-with-timeout boilerplate at the top. Per-user rate buckets keyed by (userId, provider). Fitbit handler writes tokens to wearable_connections (migration 050) on token exchange and auto-registers 3 webhook subscriptions (activities/sleep/body) with unique per-collection IDs so Fitbit can push updates; disconnect action tears subscriptions down + revokes the token. Supabase service-role REST helpers (supabaseConfig, upsertWearableConnection, getWearableConnection, deleteWearableConnection) at the top mirror the Terra pattern.
