...
│   │       ├── Import.jsx         # **Connections page** (labeled "Connections" in SideNav, URL `/connections`, internal tab id remains `import`). Dedicated page for everything that feeds Salve data: Claude Health Sync (MCP), UniversalImport auto-detect drop zone, Apple Health + MyChart expandable cards under "Medical Records", IMPORT_CATEGORIES (5 categories: Cycle/Fitness/Mood/Glucose/Genetics) with 15 app parsers under "App Exports", and **Connected Devices** section that renders the `<Wearables />` component (Oura/Dexcom/Withings/Fitbit/Whoop direct OAuth) plus the Terra multi-provider connect card. Moved here from Settings.jsx during the 2026-04-16 refactor — Settings stays focused on profile/appearance/billing/privacy. Each source card individually hideable via HideableSource. Settings has no link card pointing here anymore — SideNav key 8 is the entry point.
