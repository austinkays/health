...
│   │   ├── ai.js                 # Tiered AI service: provider routing (Gemini free / Anthropic premium via getAIProvider/setAIProvider), smart model selection per feature (getModel: lite/flash/pro tiers), feature gating (isFeatureLocked blocks Pro features on free tier; FREE_BLOCKED_FEATURES includes toolUse so chat data commands are premium-only), daily limit error handling; sendChatWithTools() agentic loop for tool-use data control (10 iteration cap, premium-gated); sends prompt_key + profile_text to server (not raw system prompts)
