...
│   │       ├── Import.jsx         # Dedicated import page: UniversalImport auto-detect drop zone at top, Apple Health + MyChart expandable cards under "Medical Records", IMPORT_CATEGORIES (5 categories: Cycle/Fitness/Mood/Glucose/Genetics) with 15 app parsers under "App Exports", Terra connected devices section. Moved from Settings.jsx to its own nav item (SideNav key 8). Each source card individually hideable via HideableSource. Settings has a link card pointing here.
│   │       ├── Legal.jsx          # Privacy Policy, Terms of Service, HIPAA Notice (tabbed interface)
│   │       └── Settings.jsx      # Appearance (theme selector: flat 4-col grid of all 16 themes — lights first then darks, alphabetical; compact tiles with 22px accent-gradient orb, name, light/dark label; signature/experimental themes self-identify via inline ✦ sparkle; free users can preview signature themes live but selection reverts on unmount), AI Provider (Gemini free / Claude premium toggle), Profile, Sage mode, pharmacy, insurance, health bg, Oura Ring connection (OAuth2 connect/disconnect, BBT baseline config, manual sync), data mgmt (backup/restore/erase), Claude sync artifact download + copyable prompt, "Import Data" link card to Import section, Support section (Report a Bug → GitHub issues, Send Feedback → in-app Feedback section)
│   └── utils/
