...
│   │   │   ├── SideNav.jsx       # Desktop sidebar navigation (hidden md:flex, 260px fixed left). App branding + user name at top, Search button with ⌘K hint + Ask Sage leaf-icon button inline in same row (icon-only, p-2.5, border), 6 nav items with active left-border accent + background tint. Replaces BottomNav at md+ breakpoint
...
- **SideNav** (desktop): Fixed left, 260px wide, full viewport height. App branding + user name at top, Search button (flex-1) + Ask Sage leaf-icon button (icon-only, `flex-shrink-0`) inline in same row with `gap-2`, 6 nav items with left-border accent on active. BottomNav hidden at md+ (`md:hidden`).
