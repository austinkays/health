...
│   ├── wearable.js               # Vercel serverless: unified OAuth2 router for ALL wearables (Oura, Dexcom, Withings, Fitbit, Whoop) via ?provider=X&action=token|refresh|data|config|status|disconnect|webhook|bootstrap_subscriptions. Consolidates 5 integrations into 1 function to stay under the Hobby tier 12-function ceiling. Each provider has its own handler block inline; shared CORS/auth/rate-limit/fetch-with-timeout boilerplate at the top. Per-user rate buckets keyed by (userId, provider). **Fitbit live-push (Phases 1-3 deployed):** token action writes to wearable_connections (migration 050) and auto-registers 3 per-user webhook subscriptions (activities/sleep/body) with unique per-collection IDs; disconnect action tears subs down + revokes token. Webhook handler responds 204 immediately (within Fitbit's strict 5s budget), then fetches the day's data via getValidFitbitAccessToken + fetchFitbitDay + transformFitbitDay and upserts into vitals (sleep/steps/active_energy/resting hr/weight) tagged source='fitbit'. Phase 3 unverified end-to-end pending a Fitbit-owning tester; follows the same Oura Phase 3 pattern that's working in production. **Oura live-push (Phases 1-3 deployed):** token action upserts wearable_connections + fetches /personal_info for provider_user_id; bootstrap_subscriptions admin action registers 7 app-level subscriptions (sleep/daily_sleep/daily_readiness/daily_activity/daily_spo2/workout/daily_stress) in oura_app_subscriptions (migration 051); renew_subscriptions admin action refreshes any sub expiring within 7 days (Vercel cron auto-trigger TODO); webhook handler (reachable at clean path /api/oura-webhook via vercel.json rewrite, since Oura strips query strings during verification) fetches incoming notifications by record id, transforms into vitals/activities tagged source='oura', upserts via service-role REST. Existing useRealtimeSync hook delivers inserts live to open browser tabs. Supabase service-role REST helpers (supabaseConfig, upsertWearableConnection, getWearableConnection, deleteWearableConnection, getConnectionByProviderUserId, patchConnectionTokens, touchConnectionWebhook, bulkInsertVitals, bulkInsertActivities, vitalAlreadyExists, activityAlreadyExists) at the top mirror the Terra pattern.
...
**Oura (section of `api/wearable.js`)** — Oura Ring V2 API proxy + live-push pipeline:
...
- **Webhook live-push (Phases 1-3 of [Oura plan](docs/superpowers/plans/2026-04-16-oura-live-push.md), all deployed):**
  ...
  - **Callback URL routing**: `/api/oura-webhook` (clean path, no query string) → rewritten in vercel.json to `/api/wearable?provider=oura&action=webhook`. The clean path is required because Oura strips query strings from callback URLs during the verification challenge.
...
```json
{
  ...
  //
  // Rewrites (vercel.json) also map two clean paths to the wearable
  // router because Oura/Fitbit may strip query strings from webhook
  // callback URLs during the verification challenge:
  //   /api/oura-webhook   → /api/wearable?provider=oura&action=webhook
  //   /api/fitbit-webhook → /api/wearable?provider=fitbit&action=webhook
