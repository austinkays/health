...
- **SideNav** (desktop): Fixed left, 260px wide, full viewport height. App branding + user name at top, Search button (standalone, full-width with ⌘K hint), 6 nav items with left-border accent on active + dimmed number key hint (1–6) on inactive items. BottomNav hidden at md+ (`md:hidden`).
...
- **Keyboard shortcuts:** `Cmd/Ctrl+K` → open search, `Escape` → close Sage popup, `1–6` → jump to Home/Medications/Vitals/Sage/Journal/Settings (blocked when a text input is focused). Number hints shown in SideNav. Implemented via global keydown listener in App.jsx.
