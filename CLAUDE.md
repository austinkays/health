...
│   ├── _wearable/shared.js       # Shared helpers for per-provider endpoints: CORS + auth + rate-limit `wrapProvider(req, res, provider, handler)` wrapper, in-memory + persistent rate limiting, `basicAuthHeader()`, all Supabase service-role REST helpers (supabaseConfig, upsertWearableConnection, getWearableConnection, deleteWearableConnection, getConnectionByProviderUserId, patchConnectionTokens, touchConnectionWebhook, bulkInsertVitals, bulkInsertActivities, vitalAlreadyExists, activityAlreadyExists, isAdminUser), and re-exports fetchWithTimeout + logUsage. Imported by oura.js/fitbit.js/dexcom.js/withings.js/whoop.js.
│   ├── oura.js                   # Vercel serverless: Oura OAuth2 proxy + live-push webhook (split out of api/wearable.js on 2026-04-17). Actions: token|refresh|data|config|status|disconnect|bootstrap_subscriptions|renew_subscriptions|webhook. **Live-push (Phases 1-3 deployed):** token action upserts wearable_connections + fetches /personal_info for provider_user_id; bootstrap_subscriptions admin action registers 7 app-level subscriptions (sleep/daily_sleep/daily_readiness/daily_activity/daily_spo2/workout/daily_stress) in oura_app_subscriptions (migration 051); renew_subscriptions admin action refreshes any sub expiring within 7 days (Vercel cron auto-trigger TODO); webhook handler (reachable at clean path /api/oura-webhook via vercel.json rewrite, since Oura strips query strings during verification) fetches incoming notifications by record id, transforms into vitals/activities tagged source='oura', upserts via service-role REST. Existing useRealtimeSync hook delivers inserts live to open browser tabs.
│   ├── fitbit.js                 # Vercel serverless: Fitbit OAuth2 proxy + live-push webhook (split out of api/wearable.js on 2026-04-17). Actions: token|refresh|data|config|status|disconnect|webhook. **Live-push (Phases 1-3 deployed):** token action writes to wearable_connections (migration 050) and auto-registers 3 per-user webhook subscriptions (activities/sleep/body) with unique per-collection IDs; disconnect action tears subs down + revokes token. Webhook handler responds 204 immediately (within Fitbit's strict 5s budget), then fetches the day's data via getValidFitbitAccessToken + fetchFitbitDay + transformFitbitDay and upserts into vitals (sleep/steps/active_energy/resting hr/weight) tagged source='fitbit'. Webhook reachable at clean path /api/fitbit-webhook via vercel.json rewrite. ⚠️ Legacy Fitbit Web API sunsets Sept 2026 — migrate to Google Health API before then.
│   ├── dexcom.js                 # Vercel serverless: Dexcom CGM OAuth2 proxy (split out of api/wearable.js on 2026-04-17). Actions: token|refresh|data|config. Sandbox vs production toggle via DEXCOM_USE_SANDBOX env var.
│   ├── withings.js               # Vercel serverless: Withings OAuth2 proxy (split out of api/wearable.js on 2026-04-17). Actions: token|refresh|data|config. Unwraps Withings' `{status: 0, body: {...}}` response envelope and translates its numeric error codes into HTTP 401 where appropriate.
│   ├── whoop.js                  # Vercel serverless: Whoop OAuth2 proxy (split out of api/wearable.js on 2026-04-17). Actions: token|refresh|data|config. Requires app approval from Whoop before credentials.
...
│       ├── 046_sage_memory.sql                    # Add sage_memory JSONB column to profiles for persistent Sage chat context
│       ├── 047_admin_theme_distribution.sql       # Extends get_admin_stats() v4 with theme_distribution key (aggregates theme_changed:* usage events, 30d window) for Admin theme popularity pie chart
│       ├── 048_admin_live_users_heatmap.sql       # Extends get_admin_stats() v5 with active_now (5-min live user count) and hourly_distribution (24-element array of {hour, events, users} from 7d usage_events, UTC)
│       ├── 049_theme_distribution_fix.sql         # Bugfix on theme_distribution aggregation in get_admin_stats()
│       ├── 050_wearable_connections.sql           # wearable_connections table (user_id, provider CHECK IN ('fitbit','oura'), provider_user_id, access_token, refresh_token, expires_at, scope, subscription_ids JSONB, status, last_webhook_at, last_sync_at, last_error). One row per (user, provider) via UNIQUE index. Server-side OAuth state so webhook handlers can act on the user's behalf without a browser session. Tokens plaintext (same threat model as terra_connections); RLS lets users SELECT+DELETE their own row, no INSERT/UPDATE policies so writes are service-role only.
│       └── 051_oura_app_subscriptions.sql         # oura_app_subscriptions table (id text PK = Oura's subscription uuid, event_type CHECK IN ('create','update','delete'), data_type, callback_url, verification_token, expiration_time, status, last_error). UNIQUE on (event_type, data_type) — one app-level subscription per pair. RLS enabled with NO policies (service-role only). Used by the Oura live-push pipeline since Oura subs are app-scoped (one POST registers a subscription that catches every Salve user's events for that pair), unlike Fitbit's per-user subscriptions stored in wearable_connections.subscription_ids.
├── src/
...
│   │   ├── themes.js             # Theme presets (single source of truth): 16 themes (7 core: lilac/noir/midnight/forest/meadow/seafoam/sunrise + 9 experimental: aurora/neon/cherry/sunbeam/blaze/ember/galactic/prismatic/crystal). Each theme: 16 hex colors + ambiance RGB (4 periods) + gradient array (3 color keys) + optional experimental:true flag; hexToRgbTriplet() utility
...
│   │   ├── ai.js                 # Tiered AI service: provider routing (Gemini free / Anthropic premium via getAIProvider/setAIProvider), smart model selection per feature (getModel: lite/flash/pro tiers), feature gating (isFeatureLocked blocks Pro features on free tier; FREE_BLOCKED_FEATURES includes toolUse so chat data commands are premium-only), daily limit error handling; sendChatWithTools() agentic loop for tool-use data control (10 iteration cap, premium-gated); getConversationCap() returns 5 for free users, Infinity for premium/admin/demo (conversation history cap); sends prompt_key + profile_text to server (not raw system prompts)
...
│   │   ├── import_promethease.js  # Promethease JSON import → genetic_results. Filters SNPs by magnitude ≥ 1.5 AND gene in PGX_GENES, derives metabolizer phenotype from summary text, cross-references PGX_INTERACTIONS for affected drugs
│   │   ├── import_23andme.js      # 23andMe raw DNA TXT import → genetic_results. Parses tab-separated genome file (rsid/chromosome/position/genotype), cross-references curated PGX_SNP_MAP of ~25 pharmacogenomic tag SNPs across 15 genes, outputs only clinically relevant variants
...
│   │   ├── oura.js               # Oura Ring integration: OAuth2 flow (getOuraAuthUrl, exchangeOuraCode), disconnectOura() (server call + localStorage clear), token storage (localStorage mirror — Phase 4 will drop it; canonical copy lives in wearable_connections), auto-refresh, data fetching (temperature/sleep/readiness/spo2/stress/workouts via /api/oura proxy for legacy manual sync), temperature deviation→BBT conversion (ouraDeviationToBBT), syncAllOuraData() bulk sync (temperature→cycles BBT, sleep/HR/SpO2/readiness/stress→vitals, workouts→activities), manual entry override protection, per-data-type dedup. **Live-push:** webhook-driven inserts also flow into vitals/activities tagged source='oura' via the api/oura.js Oura webhook handler (no client involvement); manual sync remains as a backfill path.
...
│   │   ├── dexcom.js             # Dexcom CGM client: OAuth helpers (getDexcomAuthUrl, exchangeDexcomCode), token storage + auto-refresh (single in-flight mutex), fetchDexcomEgvs(), syncDexcomGlucose() aggregates intraday EGVs to daily averages with reading count + min-max range in notes, DEXCOM_ENABLED flag. Calls /api/dexcom.
│   │   ├── withings.js           # Withings client: OAuth helpers, token storage + auto-refresh, fetchWithingsMeasurements(days), syncWithingsMeasurements() decodes numeric type codes via MEAS_TYPES constant, groups systolic+diastolic into one bp row, converts kg→lbs and C→F, WITHINGS_ENABLED flag. Calls /api/withings.
│   │   ├── fitbit.js             # Fitbit client: OAuth helpers (Basic Auth on token endpoint), syncFitbitData(days) pulls sleep+HR+steps+weight in parallel, disconnectFitbit() calls server to delete subscriptions + revoke token + clear localStorage mirror, FITBIT_ENABLED flag. Calls /api/fitbit. Tokens are dual-written (server wearable_connections + localStorage mirror) during migration; Phase 4 will drop localStorage entirely. ⚠️ Legacy API sunsets Sept 2026 — migrate to Google Health API before then.
│   │   ├── whoop.js              # Whoop client: OAuth helpers (requires 'offline' scope for refresh_token), syncWhoopData(days) pulls recoveries (HRV + RHR + recovery score) and sleep sessions, WHOOP_ENABLED flag. Calls /api/whoop. App approval required before credentials.
...
│   │   │   ├── OnboardingWizard.jsx # First-run 4-screen modal for new users. Asks "what are you tracking?" (8 focus-area checkboxes) and "what devices do you use?" (7 device checkboxes), then pre-seeds `salve:starred` Dashboard tiles and `salve:dismissed-tips` based on answers. Skippable from any screen. App.jsx shows it only when authenticated + not demoMode + not already onboarded + account looks brand new. "Brand new" means `data.settings.name` is empty AND every major table is empty (meds, conditions, vitals, journal, allergies, providers, appts, labs, todos, cycles, activities, immunizations, procedures, pharmacies, genetic_results). Any one of those signals being present triggers an auto-`markOnboardingComplete()` so a returning user signing in on a fresh browser (empty localStorage `salve:onboarded` flag) is never re-prompted. `hasCompletedOnboarding()` / `markOnboardingComplete()` / `resetOnboarding()` helpers for gating and re-run. Settings → Support has a "Re-run onboarding wizard" link.
...
│   │       ├── AIPanel.jsx       # AI Insight panel: provider badge ("via Gemini"/"via Claude") on chat responses; premium feature gating with aspirational ✦ Premium sparkle pills on tiles (full opacity/color, not greyed) + PremiumGateCard benefit cards on click (per-feature icon, description, Settings upgrade CTA); daily limit counter in amber with positive framing ("3 of 15 remaining today"), warm zero-state message; rich card-based results with accent borders (insight=lavender, connections=sage, news=amber, resources=rose, costs=sage); ResultHeader with icon badge + copy-to-clipboard + save/bookmark button; InsightResult, ConnectionsResult, NewsResult (per-story parsing with headline/body/source extraction, inline article source links, bookmark/save toggle per story via localStorage `salve:saved-news`, preamble filtering in splitSections, unbookmark confirmation), ResourcesResult, CostResult; **universal save/bookmark** for all 5 result types via `useSavedInsights` hook (localStorage `salve:saved-insights`); SaveInsightButton in ResultHeader; SavedInsightsSection collapsible on main menu; **chat as bottom-sheet overlay** (`mode === 'ask'`): `fixed inset-0 z-50` backdrop + inner panel (`78vh`, `max-w-[480px] md:max-w-[700px]`) matching SagePopup's chrome — scrollable messages area (`flex-1 overflow-y-auto min-h-0`) + pinned input/upsell footer — Escape key closes, auto-focuses input on open, focus trap; with per-message copy buttons + persistence (load/save/new chat); **conversation history panel** (Clock button next to New Chat): collapsible inline panel listing saved chats with title + relative timestamp (fmtDateRelative), tap to load, trash to delete with inline confirm/cancel, active conversation highlighted with lavender tint, free users see "X/5 saved" counter + upgrade upsell at cap, auto-prunes oldest chat when free user hits 5-conversation cap via getConversationCap(); SourcesBadges collapsible source list for web search; styled Disclaimer component; "What AI Sees" preview button at bottom of main menu; Saved News collapsible section on main menu; Saved Insights collapsible section on main menu; **FeatureLoading** breathe meditation loader (10s deep breathing cycle with star bloom 0.6→1.6x, expanding rings starting invisible, card glow pulse, "Breathe with me" + wellness messages); three-phase loading: loading→ready→revealed; "Your insight is ready" button with 2.5s fade-in + shimmer sweep; **AI prose reveal**: paragraphs fade in sequentially via `reveal` prop on AIMarkdown; ChatThinking with cycling wellness messages; **AI-powered data control** (premium-only): chat uses Anthropic tool-use API to add/update/remove health records via natural language; gated by `isFeatureLocked('toolUse')` — free users get plain chat without tools + upgrade hint below input ("✦ Upgrade for data commands"); ToolExecutionCard shows live status (pending/running/success/error/cancelled); destructive tools require inline Confirm/Cancel before execution; tool results persist in chat message history
...
│   │       ├── Admin.jsx           # Admin dashboard (tier='admin' gated via isAdminActive()): StatsPanel (DAU/WAU/MAU sparklines, AI cost tracking per-provider, section traffic, signup trends, theme popularity donut pie chart via Recharts PieChart with per-theme `lav` colors + top 8 + "Other" bucketing, live user count badge with pulse-dot + 30s auto-poll, hourly usage distribution BarChart with 24 bars opacity-scaled by activity volume), UserDrilldown (per-user activity timeline), FeedbackPanel (triage: respond/flag/delete), BetaInvitesPanel (create/list/revoke invite codes). Uses get_admin_stats() RPC.
...
│   │       ├── Import.jsx         # **Connections page** (labeled "Connections" in SideNav, URL `/connections`, internal tab id remains `import`). Reorganized 2026-04-17 into 3 state-prioritized sections: **(1) "Your connections"** at top shows everything currently active — connected OAuth wearables (rendered via `<Wearables filter={id => active.live.has(id)}>`), live Terra connections, and any source with data already in the user's tables (Apple Health / MyChart / Claude Health Sync / any of the 15 parsers whose `source` field appears in vitals/activities/cycles/journal_entries/labs/genetic_results). Empty-state card when nothing is connected yet. **(2) Universal Import** middle (unchanged `<UniversalImport>` drop zone). **(3) "Browse sources"** bottom hides what's in Section 1 and shows the rest — unconnected OAuth wearables, Terra "Connect a device" card, Medical Records (Apple Health / MyChart / Claude Health Sync), and the 5 `IMPORT_CATEGORIES` accordions (Cycle/Fitness/Mood/Glucose/Genetics) with 15 parsers total. Core primitive: module-level `computeActiveSources(data, terraConnections)` scans all tables for `source` values + cross-references `isXConnected()` localStorage mirrors, returning `{ live: Set, imported: Set, counts, terraLive }`. `connectionTick` state bumps via `onConnectionChange` callback from `<Wearables>` so OAuth connect/disconnect triggers re-classification without a full reload. `<Wearables>` is rendered twice with different `filter` props; it owns per-provider state internally and notifies the parent via a useEffect tracking all 5 `isXConnected` booleans. Card renderers (`renderAppleCard`, `renderMyChartCard`, `renderClaudeSyncCard`, `renderParserCard`, `renderFloCard`, `renderTerraConnectCard`) take an `{ imported }` flag so the same shell shows differently in Section 1 ("N records imported" subtitle + colored dot) vs Section 3 (default subtitle + dim icon). Each source individually hideable via `HideableSource` — state lifted to Import.jsx and passed into Wearables via `hiddenSources`/`onHideSource` props so both Wearables instances share one hidden set. Flo stays in the cycle category (never promoted; clicking deep-links to Cycle Tracker). Moved from Settings.jsx during the 2026-04-16 refactor — Settings stays focused on profile/appearance/billing/privacy. SideNav key 8 is the entry point.
│   │       ├── Legal.jsx          # Privacy Policy, Terms of Service, HIPAA Notice (tabbed interface)
│   │       └── Settings.jsx      # Appearance (theme selector: flat 4-col grid of all 16 themes — lights first then darks, alphabetical; compact tiles with 22px accent-gradient orb, name, light/dark label; signature/experimental themes self-identify via inline ✦ sparkle; free users can preview signature themes live but selection reverts on unmount), AI Provider (Gemini free / Claude premium toggle), Profile, Sage mode, pharmacy, insurance, health bg, BBT baseline config, data mgmt (backup/restore/erase), Support section (Report a Bug → GitHub issues, Send Feedback → in-app Feedback section). Wearable connections (Oura/Dexcom/Withings/Fitbit/Whoop OAuth + Terra), Claude Health Sync, and all data imports moved out to Connections (src/components/sections/Import.jsx) during the 2026-04-16 refactor.
│   └── utils/
...
**Supported apps (15, in addition to Apple Health + Flo + MyChart CCDA):**
...
| Promethease | `import_promethease.js` | JSON | genetic_results (pharmacogenomic SNPs, magnitude ≥ 1.5) |
| 23andMe | `import_23andme.js` | TXT | genetic_results (PGx variants from raw DNA data) |
...
**Adding a new parser:** write `src/services/import_<app>.js` following the contract above, then add one entry to the `IMPORT_CATEGORIES` array in `Import.jsx` under the appropriate category (parser module + lucide icon + tint + subtitle). No new UI code needed.
...
| `salve:oura` | `services/oura.js` | Oura OAuth tokens + expiry. **Temporary mirror** of the canonical `wearable_connections` row; Phase 4 of the Oura live-push migration will drop it once browser-side sync moves through the server. |
...
| `salve:withings` / `salve:fitbit` / `salve:whoop` | wearable services | OAuth tokens for each provider. `salve:fitbit` is a **temporary mirror** of the canonical `wearable_connections` row; Phase 4 of the Fitbit live-push migration will drop it once all browser-side API calls move through the server. |
...
- `isFeatureLocked(feature)` — returns true if feature requires premium and user is on free tier. Blocked features: connections, careGapDetect, appealDraft, immunizationSchedule, houseConsultation, monthlySummary, toolUse
...
**`api/oura.js`** — Oura Ring V2 API proxy + live-push pipeline:
- **Actions:** `token` (POST, exchanges OAuth code AND upserts wearable_connections AND fetches /personal_info to capture provider_user_id), `refresh` (POST, refreshes token AND patches the stored row), `data` (GET proxy to Oura V2 usercollection endpoints, used by the legacy client-side sync path), `config` (returns client_id + configured), `status` (GET, returns {connected, last_webhook_at, last_sync_at, expires_at, provider_user_id} for the UI), `disconnect` (POST, revokes token + deletes wearable_connections row), `bootstrap_subscriptions` (POST, admin-gated one-time setup of app-level webhook subscriptions), `webhook` (GET|POST, no user auth — Oura's servers).
- **Allowed endpoints (data action):** `daily_readiness`, `daily_sleep`, `heartrate`, `daily_spo2`, `daily_stress`, `workout`, `session`, `sleep`, `tag`, `daily_cardiovascular_age`, `daily_resilience`, `daily_activity`
- **Rate limited:** 30 requests/minute per user
- **OAuth2 flow:** Authorization code grant → `https://cloud.ouraring.com/oauth/authorize` (scope: `daily heartrate workout tag session spo2 personal email ring_configuration stress heart_health`) → callback to `/connections` with code → server exchanges for tokens (client_secret stays server-side) → server fetches `/v2/usercollection/personal_info` to get the Oura user uuid → server upserts wearable_connections row.
- **Client service:** `src/services/oura.js` — `getOuraAuthUrl()`, `exchangeOuraCode(code)`, `disconnectOura()` (server call + localStorage clear), `syncOuraTemperature(cycles, addItem, days, baseline)`, `fetchOuraTemperature(start, end)`, `ouraDeviationToBBT(deviationC, baselineF)`. Tokens are dual-written (server `wearable_connections` + localStorage mirror) during migration; Phase 4 will drop the localStorage mirror.
- **Webhook live-push (Phases 1-3 of [Oura plan](docs/superpowers/plans/2026-04-16-oura-live-push.md), all deployed):**
  - **App-level subscriptions**: unlike Fitbit's per-user subscriptions, Oura subs are app-scoped — one POST registers a subscription that catches every Salve user's events. Bootstrap action creates 7 `(create, data_type)` subscriptions (sleep, daily_sleep, daily_readiness, daily_activity, daily_spo2, workout, daily_stress) using `x-client-id` + `x-client-secret` headers (NOT user OAuth token).
  - **Verification challenge handshake**: Oura GETs (or POSTs) the callback URL during create-subscription with a `verification_token` we provided, expects 200. Implemented in `ouraWebhookHandle` GET branch; status flips to `'active'` on bootstrap success since Oura only returns 2xx after the challenge passed.
  - **Callback URL routing**: `/api/oura-webhook` (clean path, no query string) → rewritten in vercel.json to `/api/oura?action=webhook`. The clean path is required because Oura strips query strings from callback URLs during the verification challenge.
  - **Notification ingest**: incoming POST → `getConnectionByProviderUserId('oura', user_id)` → `getValidOuraAccessToken(conn)` (refreshes if expiring within 5 min, persists rotated tokens) → `fetchOuraRecord(token, data_type, object_id)` → `transformOuraRecord(data_type, record)` produces `vitals` + `activities` rows → per-row dedup against existing source='oura' rows → `bulkInsertVitals` / `bulkInsertActivities` via service-role REST → `touchConnectionWebhook` updates last_webhook_at + last_sync_at. Existing client `useRealtimeSync` hook picks up the inserts and updates the UI live.
  - **Data type mapping**: `sleep` → vitals(sleep hours + resting HR), `daily_activity` → vitals(steps + active_energy), `daily_spo2` → vitals(spo2), `workout` → activities. `daily_sleep` / `daily_readiness` / `daily_stress` are log-only for now (no clean mapping to existing vital types yet).
...
- **Data hierarchy:** Manual BBT entries override Oura-sourced entries (checked by date before inserting). Webhook-driven Oura inserts are deduped against existing source='oura' rows so they don't double-write data the user pulled via legacy manual sync.
- **Token storage:** Server-side canonical copy in `wearable_connections`; client mirror at localStorage `salve:oura` (access_token, refresh_token, expires_at, connected_at) for the legacy sync path. Auto-refresh on both sides when within 5 minutes of expiry.
- **BBT baseline:** User-configurable in Settings, stored in `localStorage` under `salve:oura-baseline` (default 97.7°F)
- **Subscription registry (`oura_app_subscriptions` table, migration 051):** one row per `(event_type, data_type)` pair, holds Oura's subscription id + verification token + expiration_time. App-level (no per-user rows). Renewal cron will be added later — Oura subscriptions expire and need periodic refresh via `PUT /v2/webhook/subscription/renew/<id>`.

> **Note on serverless function layout:** each wearable OAuth proxy is its own top-level function (`api/oura.js`, `api/dexcom.js`, `api/withings.js`, `api/fitbit.js`, `api/whoop.js`). Shared helpers (Supabase service-role REST, CORS + auth + rate-limit wrapper via `wrapProvider`, dedup, subscription bookkeeping) live in `api/_wearable/shared.js` — the `_wearable/` directory is ignored by Vercel's serverless function bundler because of the leading underscore. Terra's widget + webhook still share a single `api/terra.js` router because the two routes are tightly coupled. Prior to 2026-04-17, all five wearable providers lived in a single `api/wearable.js` router to stay under the Vercel Hobby tier 12-function ceiling; the split landed once the project moved to Vercel Pro.
...
**`api/fitbit.js`** (legacy Web API) — OAuth2:

> ⚠️ **Deprecation deadline: September 2026.** Fitbit is migrating to the new Google Health API (`health.googleapis.com/v4/users/me/`), which uses Google OAuth 2.0 and the Google Auth Library. The current Fitbit handler targets `api.fitbit.com` and will stop working in September 2026. Before that date, either rebuild `api/fitbit.js` against [developers.google.com/health](https://developers.google.com/health) or delete it. Existing tokens cannot be migrated — users will have to re-authorize. Google recommends waiting until end of May 2026 to launch Google Health API integrations to align with legacy Fitbit account deprecation.

- **Actions:** `token`, `refresh`, `data`, `config`, `status`, `disconnect`, `webhook`
...
- **Client service:** `src/services/fitbit.js` — `getFitbitAuthUrl()`, `exchangeFitbitCode(code)`, `syncFitbitData(existingVitals, addItem, days)`, `disconnectFitbit()` (server call + localStorage clear), `FITBIT_ENABLED` flag from `VITE_FITBIT_ENABLED`
...
- **Server-side OAuth state (Phase 1+2 of [live-push migration](docs/superpowers/plans/2026-04-16-fitbit-live-push.md)):** On successful `token` exchange, tokens are also upserted into `wearable_connections` (one row per user+provider) and 3 webhook subscriptions auto-register with Fitbit (activities, sleep, body). Subscription IDs are suffixed per collection (`<conn.id>-activities`, `<conn.id>-sleep`, `<conn.id>-body`) because Fitbit enforces subscription-id uniqueness across ALL of a user's subscriptions, not per-collection. Registered IDs are stored in `subscription_ids` JSONB so `disconnect` can DELETE each one from Fitbit before revoking the token and removing the row.
- **Status endpoint:** `GET ?action=status` returns `{connected, status, last_webhook_at, last_sync_at, expires_at, subscription_count, last_error}` — used by the UI to show "Last push: 2h ago" and detect error states.
- **Webhook endpoint:** `GET|POST ?action=webhook` (no user auth — Fitbit's servers). GET verifies via `FITBIT_SUBSCRIBER_VERIFY`. POST currently receives notifications and logs them but does NOT yet ingest the data — Phase 3 of the live-push plan wires webhook → fetch from Fitbit → upsert to `vitals`/`activities`. Until then, tokens are dual-written (server + localStorage mirror) so the existing manual-sync button keeps working. Phase 4 drops the localStorage mirror once the client's browser-side API calls all move server-side.
...
15. **AI-powered data control** (premium-only) - natural language CRUD via Anthropic tool-use API in chat; gated client-side by `isFeatureLocked('toolUse')` — free users get plain `sendChat()` + upgrade hint; 26 tools (add/update/remove medications, conditions, allergies, appointments, providers, todos; add vitals/journal/cycle entries/activities/genetic results; remove cycle entries; update profile; search/list records); destructive actions require inline confirmation; tool execution cards show live status; 10-iteration agentic loop cap
...
```json
{
  "functions": {
    ...
    "api/dexcom.js": { "maxDuration": 30 },
    "api/withings.js": { "maxDuration": 30 },
    "api/fitbit.js": { "maxDuration": 30 },
    "api/whoop.js": { "maxDuration": 30 },
  ...
  // 16 functions total. Each wearable provider is its own top-level
  // endpoint; shared helpers live in api/_wearable/shared.js (the
  // underscore prefix makes Vercel skip it during bundling). Terra
  // still has a single aggregated endpoint (widget + webhook share
  // logic). Previously (pre 2026-04-17) all 5 wearables lived in one
  // api/wearable.js router to stay under the Hobby 12-function cap;
  // Vercel Pro lifted that ceiling, so the providers were split back
  // out for clearer deploys and smaller per-function cold starts.
  //
  // Rewrites (vercel.json) also map two clean paths to the Oura and
  // Fitbit webhooks because both providers strip query strings from
  // callback URLs during the verification challenge:
  //   /api/oura-webhook   → /api/oura?action=webhook
  //   /api/fitbit-webhook → /api/fitbit?action=webhook
  // Register these clean paths in the provider dev consoles, never
  // the query-string variant.
  "headers": [
    {
      "source": "/(.*)",
      "headers": [
        { "key": "Content-Security-Policy", "value": "default-src 'self'; script-src 'self' 'sha256-SjlBhfGwzy+MI7Yrwc2/x4yTOV6woSAj5MFzIj2TrhI=' 'sha256-Y5Y8qULl38EZ1phbiwFtrn+8cpw/wnPK2qosRep1ZEg='; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: blob:; connect-src 'self' https://*.supabase.co wss://*.supabase.co https://fonts.googleapis.com https://fonts.gstatic.com https://cloud.ouraring.com https://api.ouraring.com https://nominatim.openstreetmap.org https://*.ingest.sentry.io https://*.ingest.us.sentry.io https://api.open-meteo.com https://geocoding-api.open-meteo.com https://api.zippopotam.us; frame-src 'none'; object-src 'none'; base-uri 'self'; form-action 'self'; worker-src 'self'; manifest-src 'self'" },
...
| **SW update prompt** | `useSWUpdate()` hook + `UpdateBanner` component. vite-plugin-pwa runs in `registerType: 'prompt'` mode so new SWs install but wait. Instead of immediately surfacing a banner, the hook silently auto-reloads at the next **safe moment**: (1) tab becomes hidden (phone lock / app switch / different tab) — always safe, catches ~95% of updates, (2) 5 minutes of idle with no focused `<input>` / `<textarea>` / contenteditable, (3) 12-hour fallback banner as a last resort if a single session stays continuously busy for half a day. A focused text field blocks auto-reload so unsaved form state (journal entry, medication form) is never clobbered. Reload path is the same as before — `updateServiceWorker(true)` posts `SKIP_WAITING` → Workbox `cleanupOutdatedCaches()` drops the old precache → `window.location.reload()` pulls fresh HTML + new content-hashed JS chunks. Encrypted localStorage data cache (`hc:cache`) persists across the reload — it's code-version-agnostic. Hook polls `registration.update()` every 60 min while visible + on visibility/focus change so long-lived tabs pick up new deploys. Meaningful updates are still surfaced via `WhatsNewModal` on the next page load, which is now automatic for most users. Dismissing the fallback banner hides the UI but leaves `needRefresh` set so silent auto-reload still kicks in later — users can't get permanently stranded on stale code. |
...
- **Signature themes** (experimental) are displayed in the same flat 4-column grid as core themes — no accordion or collapsed section. Each signature tile self-identifies via a small `✦` sparkle inline before the label. **All users (including free) can click/preview signature themes** — clicking applies the theme to the DOM immediately via `setTheme()`. Free users see a footer note: "✦ themes are preview only and reset on reload" with optional "Upgrade to save" link (gated on `BILLING_ENABLED`). When a free user leaves Settings with an unsaved signature theme active, `revertTheme()` auto-reverts to their saved theme. Premium/admin users save signature themes normally via `saveTheme()`.
...
**16 Themes (7 core + 9 experimental):**
...
- **Headings (per-theme via `.font-playfair` class targeting `html.theme-X` in `index.css`):**
  - *Soft romantic* (Lilac, Meadow, Seafoam, Sunrise): **Fraunces** variable, SOFT axis 100, weight 500
...
- **SideNav** (desktop): Fixed left, 260px wide, full viewport height. App branding + user name at top, Search button (standalone, full-width with ⌘K hint), 9 nav items (Home/Meds/Vitals/Sage/News/Scribe/Journal/Import/Settings) with left-border accent on active + dimmed number key hint (1–9) on inactive items. BottomNav hidden at md+ (`md:hidden`).
...
- **Keyboard shortcuts:** `Cmd/Ctrl+K` → open search, `Escape` → close Sage popup, `1–9` → jump to Home/Medications/Vitals/Sage/News/Scribe/Journal/Import/Settings (blocked when a text input is focused). Number hints shown in SideNav. Implemented via global keydown listener in App.jsx.
...
- [ ] AI chat: free users see plain chat (no tool-use) + "✦ Upgrade for data commands" hint below input
- [ ] AI chat: premium users get tool-use — "Add Lexapro 10mg" creates medication (ToolExecutionCard shows success)
...
- [ ] AI chat: History button (Clock icon) appears next to New Chat when saved chats exist
- [ ] AI chat: History panel lists saved conversations with title + relative date
- [ ] AI chat: Tapping a conversation in history loads it into the chat view
- [ ] AI chat: Delete button on history items shows inline confirm/cancel before removing
- [ ] AI chat: Free users see "X/5 saved" counter in history panel header
- [ ] AI chat: Free users at 5-conversation cap see upgrade upsell in history panel
- [ ] AI chat: Saving a new chat at cap auto-prunes oldest conversation for free users
- [ ] AI chat: Premium/admin users have unlimited conversation history
...
- [ ] Medications: Schedule sort buckets morning → midday → evening → bedtime → PRN correctly (buckets: 0/3/4/5, multi-dose regimens map to morning)
...
- [ ] Medications: Discontinued meds collapse behind a "N discontinued" toggle at the bottom
- [ ] Medications: Collapsed card shows reminder times + "Next in Xh Ym" when reminders exist (via utils/reminders)
- [ ] Medications: Collapsed card shows "⏰ Set reminder" inline button when no reminders; tapping expands the card and opens the time picker
- [ ] Medications: Next-dose countdown updates once per minute via nowTick useEffect
- [ ] Medications: Schedule block renders at the TOP of the expanded detail (before desktop heading) with push status chip and lavender theme
- [ ] Medications: Push status chip shows "🔔 Notifications on" when isPushSubscribed resolves true, otherwise "Set up notifications →" link to Settings
- [ ] Medications: At a glance block renders clean "Used for" sentence (no "1 INDICATIONS AND USAGE" prefix) via fdaBullet
- [ ] Medications: DailyMed deep-link chips (Side effects / Dosage / Interactions) open DailyMed in a new tab and do NOT collapse the card (stopPropagation)
- [ ] Medications: Boxed warning banner shows ≤180 chars via fdaBullet with "Read full FDA label on DailyMed →" link
- [ ] Medications: 8-section FDA accordion is gone from the codebase (grep for fdaDetailId / fdaExpanded / stripFdaHeader returns nothing)
- [ ] Medications: DiscontinuedToggle + renderMedCardBody + renderScheduleBlock helpers exist in Medications.jsx
- [ ] Sage: condenseFDA output for a med with FDA data includes "used for / dosing / precautions" strings (verify with scripts/verify-condense-fda.mjs)
- [ ] Sage: fdaBullet strips "1 INDICATIONS AND USAGE" style prefixes without touching short acronyms like "MRI" or "HIV"
- [ ] Sage: existing condenseFDA field budgets from commit 5db1977 (140/100/100/120/80) are preserved
- [ ] Helpers: `node scripts/verify-condense-fda.mjs` passes 26 assertions
- [ ] Helpers: `node scripts/verify-reminders-util.mjs` passes 17 assertions
...
- [ ] AI chat: "Add a todo to refill prescription" creates todo via tool-use (premium only)
- [ ] AI chat: "Mark my refill todo as complete" updates todo (premium only)
- [ ] AI chat: "Remove todo" requires confirmation before deleting (premium only)
...
- [ ] Admin: StatsPanel shows theme popularity donut pie chart (top 8 + "Other") with per-theme lav colors
...
| `OURA_WEBHOOK_CALLBACK_URL` | Vercel env vars (optional) | Pin Oura webhook callback to a specific URL (e.g. `https://salve.today/api/oura-webhook`) instead of deriving from request host. Useful if a preview deploy ever calls bootstrap_subscriptions and would otherwise register its own preview URL with Oura. |
...
| `FITBIT_SUBSCRIBER_VERIFY` | Vercel env vars only | Value Fitbit sends as the `verify` query param when confirming a subscriber endpoint; `api/fitbit.js` echoes a 204 when the incoming value matches. Required only if you enable Fitbit webhook subscriptions. |
...
- [ ] **Today's doses checklist (heavier reminders version)** — Surface a top-of-page "Today" block in Medications that lists every reminder firing today (sorted by time), with a "Taken ✓" button per dose that writes a lightweight adherence record and optionally a journal entry tagged with the med and timestamp. Requires: `medication_reminders.days_of_week` column (new migration) so users can say "weekdays only", a new `dose_logs` table (or a journal-entry convention) to record taken/missed events, extension of the existing `/api/cron-reminders.js` + `Push.js` flow to per-reminder-time delivery with action buttons (Take / Snooze / Skip), and Dashboard quick-access integration. Turns Medications from a reference page into an active daily tool. Deferred from the 2026-04-15 Medications redesign — the lighter version (card-surface reminder row, next-dose countdown, Schedule block at top of detail pane) landed with that redesign.
- [ ] **Ship the live-push What's New entry** — A draft is already written and sitting commented-out at the top of [src/constants/changelog.js](src/constants/changelog.js). To publish: uncomment, paste as the first item in the CHANGELOG array, set the date, and bump `CURRENT_WHATS_NEW_ID` to `'1.3.0-beta.4'`. The modal will auto-open for everyone on next visit. Holding off on shipping until Fitbit Phase 3 is confirmed working with a real Fitbit user (so the entry doesn't promise something that fails for them).
- [ ] **Wearable live-push Phase 4 (client simplification)** — `isOuraConnected()` / `isFitbitConnected()` go async and call the new server `?action=status` instead of reading localStorage; drop the `salve:oura` / `salve:fitbit` localStorage mirrors entirely; add a "Last push: 2h ago" pill on the Oura/Fitbit cards in Connections sourced from `last_webhook_at`. Mostly polish + reducing dual-write complexity. Plans: [Oura](docs/superpowers/plans/2026-04-16-oura-live-push.md) Phase 4, [Fitbit](docs/superpowers/plans/2026-04-16-fitbit-live-push.md) Phase 4.
- [ ] **Wire Oura subscription renewal cron** — The `renew_subscriptions` admin action is shipped (in `api/oura.js`) but currently requires manual triggering. Add a weekly Vercel cron entry to vercel.json that hits the endpoint with CRON_SECRET-style auth so subs never silently expire. Without this, Oura subscriptions go stale after their expiration_time and notifications stop firing without warning.
