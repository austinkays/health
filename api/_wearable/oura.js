// ── Oura Ring ────────────────────────────────────────────────────────
// OAuth2 token/refresh + data proxy + live-push webhook ingestion.
// App-level subscriptions (not per-user) — one POST registers a sub
// that catches every Salve user's events. See docs/superpowers/plans/
// 2026-04-16-oura-live-push.md for the full Phase 1-4 migration.
...
export async function ouraHandle(action, req, res, userId) {
  const clientId = process.env.OURA_CLIENT_ID;
  const clientSecret = process.env.OURA_CLIENT_SECRET;
...
export async function ouraWebhookHandle(req, res) {
