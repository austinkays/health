// ── Fitbit — OAuth2 proxy + live-push webhook ─────────────────────
// Split out of api/wearable.js on 2026-04-17. See api/_wearable/shared.js
// for rate-limit, CORS, auth wrapper, and Supabase service-role helpers.
//
// ⚠️  DEPRECATION WARNING — legacy Fitbit Web API sunsets September 2026.
// Must migrate to Google Health API before then. See CLAUDE.md.
...
async function fitbitHandle(action, req, res, userId) {
  ...
  if (action === 'token') {
    ...
    // its own /api/fitbit?action=data calls until Phase 4 moves sync
...
//   https://<your-domain>/api/fitbit-webhook
// (rewritten in vercel.json to /api/fitbit?action=webhook)
//
...
export default function handler(req, res) {
  ...
  if (action === 'webhook') return fitbitWebhookHandle(req, res);
  return wrapProvider(req, res, 'fitbit', fitbitHandle);
}
