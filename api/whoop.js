// ── Whoop API proxy — handles OAuth2 + recovery/sleep/cycle data ──
// Keeps client_secret server-side. All calls require Supabase auth.
//
// Whoop is the gold standard for HRV/recovery tracking — extremely valuable
// for the dysautonomia / POTS audience where autonomic markers (HRV, RHR,
// respiratory rate) are core to symptom tracking. Free developer API.
//
// Actions:
//   token   — exchange auth code for tokens (POST)
//   refresh — refresh expired token (POST)
//   data    — proxy GET to Whoop v1 API (GET)
//   config  — return client_id + configured (GET)
...
export default async function handler(req, res) {
  ...
  if (!(await checkPersistentRateLimit(userId, 'whoop', RATE_LIMIT_MAX, 60))) {
  ...
  try {
    ...
    // ── Proxy data requests to Whoop API ──
    if (action === 'data') {
      ...
      const allowed = [
        'v1/cycle',           // daily cycles (strain)
        'v1/recovery',        // HRV + RHR + recovery score
        'v1/activity/sleep',  // sleep details
      ...
      const url = `${WHOOP_API_BASE}/${ep}${params.toString() ? '?' + params : ''}`;
    ...
    if (e.name === 'AbortError') return res.status(504).json({ error: 'Whoop API timeout' });
